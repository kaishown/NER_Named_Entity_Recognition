...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    embedding = model.get_sequence_output() # 获得bert输出的结果作为embeding
    max_seq_length = embedding.shape[1].value

    used = tf.sign(tf.abs(input_ids)) # tf.sign(x)符号函数，小于0为-1，等于0 为0，大于0为1 #tf.abs()绝对值函数
    lengths = tf.reduce_sum(used, reduction_indices=1)  # 塌维加法，按某一维度进行加法；[batch_size] 大小的向量，包含了当前batch中的序列长度
...
def main():
    tf.logging.set_verbosity(tf.logging.INFO)  # 运行代码时，将会看到info日志输出
    processors = {
        "ner": NerProcessor  # 对应一个数据类
    }
    ...
    if FLAGS.max_seq_length > bert_config.max_position_embeddings:  # 最大序列长度不能大于预训练模型中定义的长度512
        raise ValueError(
    ...
    if FLAGS.clean and FLAGS.do_train:
        if os.path.exists(FLAGS.output_dir):
            ...
            try:
                ...
                exit(-1)
        if os.path.exists(FLAGS.data_config_path):  # 训练和评估保存的data.confg数据存在则删除
            try:
                ...
                print('pleace remove the files of output dir and data.conf')
                exit(-1)
    task_name = FLAGS.task_name.lower()  # 加载训练任务
    if task_name not in processors:  # 确保是ner任务
        raise ValueError("Task not found: %s" % (task_name))
    # 2--------数据处理成bert需要的形式---------
    processor = processors[task_name]()

    label_list = processor.get_labels()  # 可以提前在get_label方法中定义标签
    # FullTokenizer的作用就很显而易见了，对一个文本段进行以上两种解析，最后返回词（字）的数组，
    # 同时还提供token到id的索引以及id到token的索引。这里的token可以理解为文本段处理过后的最小单元
    tokenizer = tokenization.FullTokenizer( # 对bert的词汇文件在进行变小写后进行fulltokenizer
        vocab_file=FLAGS.vocab_file, do_lower_case=FLAGS.do_lower_case)
    tpu_cluster_resolver = None  # 不使用tpu集群
    if FLAGS.use_tpu and FLAGS.tpu_name:  # 不考虑
        tpu_cluster_resolver = tf.contrib.cluster_resolver.TPUClusterResolver(
            FLAGS.tpu_name, zone=FLAGS.tpu_zone, project=FLAGS.gcp_project)
    # 如果为PER_HOST_V1或PER_HOST_V2，则在每个主机上调用一次input_fn。
    # #使用每核心输入管道配置，每个核心调用一次。 具有全局批量大小
    is_per_host = tf.contrib.tpu.InputPipelineConfig.PER_HOST_V2  #
    # 定义tpu函数
    run_config = tf.contrib.tpu.RunConfig(
        cluster=tpu_cluster_resolver,
        master=FLAGS.master,
        model_dir=FLAGS.output_dir,  # 输出文件位置
    ...
    if FLAGS.do_train:
        ...
        if len(data_config) == 0:  # 如果为空
            ...
            num_train_steps = int(  # 训练执行的总批次为总样本个数/训练总批次*训练总次数
                len(train_examples) / FLAGS.train_batch_size * FLAGS.num_train_epochs)
            num_warmup_steps = int(num_train_steps * FLAGS.warmup_proportion)  # num_train_steps*线性学习率热身训练的比例

            data_config['num_train_steps'] = num_train_steps  # 训练批次
            data_config['num_warmup_steps'] = num_warmup_steps  # ??
            data_config['num_train_size'] = len(train_examples)  # 总样本数
        else:
    ...
    model_fn = model_fn_builder(
        ...
        init_checkpoint=FLAGS.init_checkpoint,  # 初始检查点（通常来自预先训练的bert模型）
        learning_rate=FLAGS.learning_rate,  # 学习率5e-5
        num_train_steps=num_train_steps,  # 总批次
        num_warmup_steps=num_warmup_steps,  # warmup数
        use_tpu=FLAGS.use_tpu,  # 不使用tpu
        use_one_hot_embeddings=FLAGS.use_tpu)  # none

    estimator = tf.contrib.tpu.TPUEstimator(  # 定义评估器
        use_tpu=FLAGS.use_tpu,  # none
        model_fn=model_fn,  # 上面定义的model
        config=run_config,  # 上面定义的run_config参数
    ...
    if FLAGS.do_train:
        ...
        if data_config.get('train.tf_record_path', '') == '':
            ...
            filed_based_convert_examples_to_features(  # 输入样本、标签、最大长度、tokenizer,数据
        ...
        train_input_fn = file_based_input_fn_builder(
            input_file=train_file,  # 训练数据
            seq_length=FLAGS.max_seq_length,  # 最大长度 128
    ...
    if FLAGS.do_eval:  # 进行评估
        if data_config.get('eval.tf_record_path', '') == '':  # 如果字典中没有评估路径
            eval_examples = processor.get_dev_examples(FLAGS.data_dir)  # 对到data_dir的dev文件
            eval_file = os.path.join(FLAGS.output_dir, "eval.tf_record")  # 输出得到eval.tf_record文件
            filed_based_convert_examples_to_features(  # 将评估文件转换
                eval_examples, label_list, FLAGS.max_seq_length, tokenizer, eval_file)
            data_config['eval.tf_record_path'] = eval_file  # 将评估文件加入数据
            data_config['num_eval_size'] = len(eval_examples)  # 将评估文件长度加入数据
        else:
            eval_file = data_config['eval.tf_record_path']  # 将评估文件数据读出
        # 打印验证集数据信息
        num_eval_size = data_config.get('num_eval_size', 0)  # 评估文件长度

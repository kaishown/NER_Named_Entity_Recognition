...
root_path = r'./'
output_path = 'output'
origin_test_txt = './testdata/t1.txt'
...
## Other parameters
flags.DEFINE_string(
    # "init_checkpoint", os.path.join(output_path, 'model.ckpt'),
    "init_checkpoint", output_path,
...
flags.DEFINE_float("num_train_epochs", None, "Total number of training epochs to perform.")
flags.DEFINE_float('droupout_rate', None, 'Dropout rate')
...
class test_data_preprocess(object):
    """
    Converts data to a specific format and saves it
    """
    ...
    def save_txt(self, lines, txt_path):
        with codecs.open(txt_path, 'w', encoding='utf-8') as f:
            f.writelines(lines)
            print("=======保存文件成功========目标文件为{}".format(txt_path))

    def txt_to_lines(self, txt_path):
        # print(txt_path)
        lines = []
        for txt in self.read_txt(txt_path):
            lines.extend([i + ' O\n' for i in txt.strip()])
            # print(lines, txt)
            lines.append('\n')
        save_path = os.path.join(FLAGS.data_dir, 'test.txt')
        # save_path = 'ttt.txt'
        self.save_txt(lines=lines, txt_path=save_path)
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    lengths = tf.reduce_sum(used, reduction_indices=1)  #  [batch_size] 大小的向量，表示当前batch中的序列实际长度
...
def main():
    ...
    test_data_preprocess().txt_to_lines(origin_test_txt) #添加的文件转换脚本
    processors = {
    ...
    tokenizer = tokenization.FullTokenizer(  # 建立字典 （字符和id）
    ...
    if FLAGS.do_predict:
        ...
        def result_to_pair(writer):
            for predict_line, prediction in zip(predict_examples, result):
                ...
                print("line_toke{}".format(line_token))
                print("label_token{}".format(label_token))
